let Caniuse = {
  ...
  mounted () {
    let self = this
    let caniuseUrl = `https://raw.githubusercontent.com/Fyrd/caniuse/master/features-json/${self.features}.json`

    fetch(caniuseUrl).then(res => res.json()).then(res => {
      let supports = res.stats

      self.browserTitle = res.title
      self.browserCategories = res.categories

      for (let i = 0, len = self.browsers.length; i < len; i++) {
        let browser = self.browsers[i].name
        let verions = supports[browser]
        for (let version in verions) {
          if (verions[version].indexOf('y') > -1 || verions[version].indexOf('x') > -1) {
            self.browsers[i].version = version
            break
          }
        }
      }
