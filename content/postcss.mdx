...
**Chú ý:** _Bạn có thể đổi `[input.css]` và `[output.css]` thành tên tập tin bạn mong muốn._
...
Ngoài việc định nghĩa biến bằng các thuộc tính CSS. [**PostCSS Preset Env**](https://preset-env.cssdb.org/) còn có những biến định nghĩa _media queries_, _selectors_, các chức năng về _color function_, _pseudo-class_, _nesting rules_... Bên cạnh đó còn giúp chuyển đổi các CSS hiện đại thành CSS mà hầu hết trình duyệt đều hỗ trợ.
...
- [CSS MQPacker](https://github.com/hail2u/node-css-mqpacker): đóng gói các quy tắc về _media queries_ giống nhau về thành một
...
- [CSSNano](https://cssnano.co/): Tối ưu code CSS như _minification_, _compresses inline SVG_, _duplicates_, _comments_, _unused rule_...
